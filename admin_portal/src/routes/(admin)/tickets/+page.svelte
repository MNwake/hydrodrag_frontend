<script lang="ts">
	...
	let scanResult: {
		...
		error?: string;
		used_at?: string | null;
		/** Whether this result is from scan or undo (for success message). */
		source?: 'scan' | 'undo';
	} | null = null;
	...
	async function handleScan(codeOverride?: string) {
		...
		try {
			...
			qrScanner?.start(); // Resume camera on error
		} finally {
			scanLoading = false;
		}
	}
	...
	async function openCameraScan() {
		...
		cameraScanOpen = true;
		scanResult = null;
		await tick(); // Wait for DOM to render video element
		...
		try {
			...
			qrScanner = new QrScanner(
				videoEl,
				(result) => {
					if (scanLoading) return; // Ignore while API request in progress
					const code = extractTicketCode(result.data);
					if (!code) return;
					qrScanner?.pause(); // Stop scanning to avoid duplicate callbacks
					handleScan(code);
				},
				{
					returnDetailedScanResult: true,
					preferredCamera: 'environment', // Back camera on mobile for easier scanning
	...
	/** Purchaser: racer full name when racer is hydrated, else purchaser_name. */
...
<div class="page-header">
	<h1 class="page-title">Spectator tickets</h1>
	<p class="page-subtitle">List and scan spectator tickets.</p>
</div>
...
<svelte:window on:keydown={handleCameraKeydown} />
...
<style>
	...
	.camera-video-wrap :global(video + div) {
		/* qr-scanner overlay for scan region */
